...
Global lbfont:TFont
Global score_path:String="C:/Users/Yusuf/repos/Z-WAVE-development/Z-WAVE - v3/Game files/zwave.data/scorefile.txt"
...
Class Game_app Extends App
	Field player:Player
	Field playertwo:Player
	Field raygunbullet_collection:List<RayGunBullet>
	Field raygunbullet_collection_playertwo:List<RayGunBullet>
	Field enemy_collection:List<Zombie>
	Field menu:Image
	Field menu_hoff:Image
	Field menu_hon:Image
	Field help:Image
	Field help2:Image
	Field help3:Image
	...
	'2nd
	Field score_playertwo:Int
	Field points_playertwo:Int
	
	Field raygunbullet_count:Int
	
	'2nd
	Field raygunbullet_count_playertwo:Int
	...
	'2nd player's
	Field reload_state_playertwo:String
	Field reload_cycle_playertwo:Int
	Field health_playertwo:Int
	...
	'2nd player's
	Field enemy_collision_playertwo:Bool
	Field player_collision_playertwo:Bool
	...
	'2nd player's
	Field tick_health_playertwo:Int
	Field tick_reload_playertwo:Int
	Field regenerate_playertwo:Bool
	...
	'2nd
	Field direction_playertwo:Bool
	...
	'2nd
	Field name_playertwo:String
	Field name_given_playertwo:String
	...
	'2nd
	Field gun_damage_playertwo:Int
	Field PaP_upgrades_playertwo:Int
	Field weapon_damage_playertwo:Int
	Field tick_PaP_playertwo:Int
	Field weapon_upgradeCooldown_playertwo:Bool
	Field PaP_cost_playertwo:Int
	Field PaP_cycle_playertwo:Int
	
	'back to game/logic variables
	Field hardmode:Bool
	Field hardmode_toggle:Int
	Field hardmode_allow:Bool=False
	Field lobby_tick:Int
	Field lobby_tick_save:Int
	...
	Method OnCreate ()
		...
		menu_hoff = LoadImage ("menu_hoff.png")
		menu_hon = LoadImage ("menu_hon.png")
	...
	'All the game logic
		...
		Select GameState
			Case "MENU"
				'If lobby_tick<24 Then lobby_tick+=1
				'If lobby_tick=24 Then lobby_tick=0
				
				'If lobby_tick_save=lobby_tick And hardmode_allow=False Then
				'	lobby_tick_save=99999999
				'	hardmode_allow=True
				'End
				...
				If KeyHit(KEY_E) Then 'and hardmode_allow=True
					hardmode=True
					hardmode_toggle+=1
					If hardmode_toggle>1 Then
						hardmode=False
						hardmode_toggle=0
					End
				End
		...
		'the zombies required to kill are negative and zombies in round is negative then 
			...
			Case "PLAYING"
				...
				If tick<120 Then tick+=1
				If tick=120 Then tick=0
			...
			' within a 2 second window of the last one
				...
				player_collision=False ' duplicated from below
				For Local enemy:=Eachin enemy_collection
					If intersects(player.x,player.y,20,22,enemy.x,enemy.y,22,27) Then player_collision=True ' duplicated from below
				Next
				
				player.Move(GameState,tick,player_collision,hardmode)
				...
				'the new code that was above before
					...
					'If intersects(player.x,player.y,20,22,enemy.x,enemy.y,22,27) Then player_collision=True ' moved below
					enemy.Move(player.give_x(),player.give_y(),GameState,enemy_collision,player_collision,hardmode,tick)
	...
	'All the graphics drawing
		...
		Select GameState
			Case "MENU"
				If hardmode=0 Then DrawImage menu_hoff, 0,0 'hard mode off image
				If hardmode=1 Then DrawImage menu_hon, 0,0 'hard mode on image
				
			Case "HELP"
				If helpscreen=0 Then
					DrawImage help, 0,0 ' first help image
					DrawImage helpoverlay, 0,0 ' the overlay that allows for a cleaner look beteen screens
				End
				If helpscreen=1 Then
					DrawImage help2, 0,0 ' second help image
					DrawImage helpoverlay, 0,0 ' same help overlay for continuity
				End
				If helpscreen=2 Then
					DrawImage help3, 0,0 ' third help image
					DrawImage helpoverlay, 0,0 ' same help overlay for continuity
				End
			...
			Case "NewRound"
				Cls 255, 255, 255
				SetColor 0,0,0
				If direction=True Then DrawImage player.sprite, player.x, player.y ' the player sprite will not disappear, so the user will not lose them Note a mouse cursor getting lost
				If direction=False Then DrawImage player.sprite2, player.x, player.y ' the left direction for the same player model but mirrored for left
			Case "Leaderboard"
...
Class Player
	...
	Field x:Float = 300
	Field y:Float = 218
	Field Player_speed:Float = 1.5
	Field Player_speed_sprint:Float = 3
	...
	Field four_option:Float = Rnd(0,4)
	Field huff_sound:Sound=LoadSound("huff.ogg")
	Field x_old:Float
	Field y_old:Float
	Field xspeed:Float=0
	Field yspeed:Float=0
	Field increm:Float=0.5
	Field sprint_multiplier:Float = 3/1.5
	Field x_disp:Float
	Field y_disp:Float
	...
	Method Move(GameState:String,tick:Int,player_collision:Bool,hardmode:Bool)
		If hardmode=True And player_collision=True Then
			Player_speed=0
			xspeed=0
			yspeed=0
			up=False
		...
		'If KeyDown(KEY_SHIFT) And GameState="PLAYING" And sprint_energy>0 And ( KeyDown(KEY_W) Or KeyDown(KEY_A) Or KeyDown(KEY_S) Or KeyDown(KEY_D) ) Then travel+=resultant_disp
		
		If travel>35 And GameState = "PLAYING" Then
			'four_option=Rnd(0,4)
			'If 0<=four_option And four_option>1 Then PlaySound(footstep1_sound,8)
			'If 1<=four_option And four_option>2 Then PlaySound(footstep1_sound,8)
			'If 2<=four_option And four_option>3 Then PlaySound(footstep1_sound,8)
			'If 3<=four_option And four_option>4 Then PlaySound(footstep1_sound,8)
...
Class Zombie
	...
	Method New()
		...
		If Rnd(0,100)>70 Then Zombie_speed=2.5 'sprinters
		
	End
	
	
	Method Move(playerx:Float,playery:Float,GameState:String,enemy_collision:Bool,player_collision:Bool,hardmode:Bool,tick:Int)
		...
		If player_collision=True And hardmode=False Then
		...
		If enemy_collision=False And Zombie_speed<4.0 Then Zombie_speed=1.2
	End
...
Class RayGunBullet
	...
	Field up:Bool
	Field down:Bool
	Field RayGunBullet_speed:Int = 13
...
Function writenamesandscore:Int(name:String,score:Int)
	...
	score_file=FileStream.Open(score_path,"a")
...
Function toptenplayers() ' top ten function
	...
	scores_file= FileStream.Open(score_path,"r")
